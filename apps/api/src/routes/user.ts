...
import type { JwtData, ProfileData } from '@shared/types/tv-tracker.js';
...
import { scheduleEpisodeUpdate } from '../utils/jobQueue.js';
...
const user = new Hono<{ Bindings: Bindings; Variables: Variables }>()
  ...
  .post('/tvshow', zValidator('json', tvMazeShowBodySchema, validationHook), async c => {
    try {
      ...
      scheduleEpisodeUpdate(c.executionCtx, db, showData, newShowId);
  ...
  .post('/tvshow/:id', zValidator('param', numericIdParamSchema, validationHook), async c => {
    ...
    try {
      ...
      const response = await fetch(
        `${TV_MAZE_API_BASE}/shows/${tvMazeId}?embed[]=nextepisode&embed[]=previousepisode`,
  ...
  .patch('/tvshow/:id', zValidator('param', numericIdParamSchema, validationHook), async c => {
    ...
    try {
      ...
      const response = await fetch(
        `${TV_MAZE_API_BASE}/shows/${existing[0].tvMazeId}?embed[]=nextepisode&embed[]=previousepisode`,
        { signal: AbortSignal.timeout(8000) },
      );
      if (!response.ok) {
        return c.json(err(`TvMaze response status: ${response.status}`), 502);
